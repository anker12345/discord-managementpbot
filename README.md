...
*   **サーバーの初期構築が面倒**: `config.yaml`ファイルまたはファイルアップロードで、チャンネル・カテゴリ・権限を持つロール（基幹ロール）のすべてを一度に構築します。
...
*   **YAMLによるサーバー構築**: 設定ファイルまたはファイルアップロードからサーバーの骨格を何度でも再現。
*   **ファイルアップロード対応**: コマンド実行時に直接YAMLファイルをアップロードしてセットアップ可能。
*   **設定ファイル検証機能**: セットアップ前に設定ファイルの妥当性をチェック。
...
| `/setup_file <config_file> [force]` | アップロードしたYAMLファイルに基づいてサーバーを構築します。 |
| `/validate_config <config_file>` | アップロードしたYAMLファイルの妥当性をチェックします。 |
...
1. **設定ファイルの準備**: YAMLファイル（.yaml または .yml）を作成
2. **妥当性チェック**: `/validate_config` でファイルをアップロードして検証
3. **サーバーセットアップ**: `/setup_file` でファイルをアップロードして構築
...
`utils/validators.py` に新しいバリデーション関数を追加して、設定ファイルの検証を強化できます。
